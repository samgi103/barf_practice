...
def parse_arguments(args):
    log.sam_title("[options] parse arguments start")
    """
    ...
    log.sam_info(f"opt_cmd : {opt_cmd}")
    log.sam_title("[options] parse arguments end")
    return opt_cmd


def set(opt_cmd={}):
    log.sam_title("[options] set start")
    log.info("setting configurations...")
    assert("model" in opt_cmd)
    # load config from yaml file
    assert("yaml" in opt_cmd)
    log.sam_info(f"opt_cmd.yaml = {opt_cmd.yaml} -> load options/{opt_cmd.yaml}/yaml")
